...
function pathToWorkspaceUri(folderPath: string): string {
    ...
    let rest = p;
    if (p.length >= 2 && p[1] === ':') {
        drive = p[0].toLowerCase();
        rest = p.slice(2);
    }
    const segments = rest.split('/');
    const encodedSegments = segments.map(seg => encodeURIComponent(seg).replace(/'/g, '%27'));
    const encodedPath = encodedSegments.join('/');

    if (drive) {
        return `file:///${drive}%3A${encodedPath}`;
    } else {
...
export function inferWorkspaceFromBrain(conversationId: string, brainDir = getBrainDir()): string | null {
    ...
    const pathPattern = SYSTEM === 'Windows'
        ? /file:\/\/\/([A-Za-z](?:%3A|:)\/[^\s"'>]+)/g
        : /file:\/\/\/([^\s"'>]+)/g;
        ...
        for (const name of files) {
            ...
            try {
                ...
                while ((match = pathPattern.exec(content)) !== null) {
                    let raw = match[1];
                    raw = raw.replace(/%3A/gi, ':').replace(/%20/g, ' ').replace(/\\/g, '/');
...
export function hasTimestampFields(innerBlob: Buffer | null): boolean {
    ...
    try {
        ...
        while (pos < innerBlob.length) {
            const [tag, tagPos] = decodeVarint(innerBlob, pos);
            const fn = tag >>> 3;
            if (fn === 3 || fn === 7 || fn === 10) return true;
            pos = skipProtobufField(innerBlob, tagPos, tag & 7);
        }
    } catch (e) { }
    return false;
}

export function extractExistingMetadata(dbPath: string): { titles: Record<string, string>, innerBlobs: Record<string, Buffer> } {
...
export function getTitleFromBrain(conversationId: string, brainDir = getBrainDir()): string | null {
    const brainPath = path.join(brainDir, conversationId);
    if (!fs.existsSync(brainPath) || !fs.statSync(brainPath).isDirectory()) return null;

    try {
        const files = fs.readdirSync(brainPath).filter(f => f.endsWith('.md') && !f.startsWith('.'));
        files.sort();
        for (const f of files) {
            try {
                const content = fs.readFileSync(path.join(brainPath, f), 'utf-8');
                const firstLine = content.split('\n')[0].trim();
                if (firstLine.startsWith('#')) return firstLine.replace(/^#\s*/, '').substring(0, 80);
            } catch (e) { }
        }
    } catch (e) { }
    return null;
...
export function resolveTitle(
    ...
    if (existingTitles[conversationId]) return [existingTitles[conversationId], 'preserved'];

    const convFile = path.join(conversationsDir, `${conversationId}.pb`);
    if (fs.existsSync(convFile)) {
        const d = new Date(fs.statSync(convFile).mtimeMs);
        const mStr = d.toLocaleDateString('en-US', { month: 'short', day: '2-digit' });
...
export function buildTrajectoryEntry(
    ...
    if (existingInnerData) {
        ...
        if (workspacePath) innerInfo = Buffer.concat([innerInfo, buildWorkspaceField(workspacePath)]);
        if (pbMtime) innerInfo = Buffer.concat([innerInfo, buildTimestampFields(pbMtime)]);
    }
